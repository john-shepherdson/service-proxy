...
public class MapRealmAdapter extends AbstractRealmModel<MapRealmEntity> implements RealmModel {
    ...
    public IdentityProviderModel getIdentityProviderById(String internalId) {
        MapIdentityProviderEntity e = entity.getIdentityProviders().stream().filter(idp->internalId.equals(idp.getId())).findAny().orElse(null);
        return MapIdentityProviderEntity.toModel(e, () -> this.getModelFromProviderFactory(e.getProviderId()));
    }

    @Override
    public Stream<IdentityProviderModel> getAutoUpdatedIdentityProvidersStream() {
        return entity.getIdentityProviders().stream().filter(idp -> idp.getConfig() != null && idp.getConfig().get(IdentityProviderModel.METADATA_URL) != null).map(e -> MapIdentityProviderEntity.toModel(e, () -> this.getModelFromProviderFactory(e.getProviderId())));
    }
