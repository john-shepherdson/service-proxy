...
import { Loading } from "../LoadingModal.js";
export const GroupAdmins = props => {
  ...
  useEffect(() => {
    ...
    props.groupConfiguration?.admins?.length > 0 && props.groupConfiguration?.admins.map(admin => {
      groupadminids.push(admin.user.id); // groupadminids.push(admin.user.id);
    });
  ...
  let fetchGroupAdminIds = () => {
    groupsService.doGet("/group-admin/groupids/all").then(response => {
      if (response.status === 200 && response.data) {
        setGroupIds(response.data); // setGroupMembers(response.data.results);
      }
    }).catch(err => {
      console.log(err);
    });
  ...
  let fetchGroupMembers = async (searchString = "") => {
    groupsService.doGet("/group-admin/groups/members", {
      params: {
        max: 20,
        search: searchString,
      ...
      if (response.status === 200 && response.data) {
        ...
        response.data.results.forEach(membership => {
          members.push({
            value: getUserIdentifier(membership),
            description: membership.email,
            id: membership.id,
            disabled: groupAdminIds.includes(membership.id)
          });
        });
